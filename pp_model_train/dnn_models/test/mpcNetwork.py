...
def generateData(data, frames=1): 
    ...
    print(episodes)
    input_data = np.zeros((data.shape[0]*frames, xk_data.shape[1]-episodes))
    ...
    for episode in range(episodes):
        ...
        for i in range(999):
            # Stack up frames
            if i >= frames:
                input_k = data[:,1000*episode+i-frames:1000*episode+i].flatten()
            else:
                input_k = data[:,1000*episode+i]
            print(input_k.shape)
            input_data[:, 999*episode+i] = data[:,1000*episode+i]
            #print('input', input_data.shape)
            output_data[:, 999*episode+i] = data[:4,1000*episode+i+1]
