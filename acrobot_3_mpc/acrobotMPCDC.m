...
%Solving
theta = x0;
for ct = 1:params.M-params.N
    sprintf('iteration #: %d', ct)
    uopt = zeros(1,params.N);
    p1 = p(:,ct:ct+params.N-1);
    COSTFUN = @(p1) acrobotObjectiveFCN(p1, xf, uopt(:,1), params);
    CONSFUN = @(p1) acrobotConstraintFCN_DC(p, x0, xf, params);
    p1 = fmincon(COSTFUN, p1, [], [], [], [], LB, UB, CONSFUN, options);
